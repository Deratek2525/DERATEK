...
function searchLocataire(query) {
  const results = $('r-locataire-results');
  if (!results) return;
  if (!query || query.length < 1) { results.style.display = 'none'; return; }
  const q = query.toLowerCase();
  const matches = DB.locataires.filter(l =>
    (l.prenom + ' ' + l.nom + ' ' + (l.ville||'') + ' ' + (l.adresse||'')).toLowerCase().includes(q)
  ).slice(0, 10);
  if (!matches.length) {
    results.style.display = 'block';
    results.innerHTML = '<div style="padding:10px;color:var(--g400);font-size:13px;">Aucun locataire trouvé</div>';
    return;
  }
  results.style.display = 'block';
  results.innerHTML = matches.map(l => `
    <div onclick="selectLocataire('${l.id}')" style="padding:10px 14px;cursor:pointer;border-bottom:1px solid var(--g100);display:flex;justify-content:space-between;align-items:center;" onmouseover="this.style.background='#f0f4ff'" onmouseout="this.style.background='#fff'">
      <div>
        <div style="font-weight:700;font-size:13px;">${l.prenom} ${l.nom}</div>
        <div style="font-size:11px;color:var(--g400);">${l.adresse||''} ${l.npa||''} ${l.ville||''}</div>
      </div>
      <div style="font-size:11px;color:var(--g400);">${l.tel||''}</div>
    </div>`).join('');
}

function selectLocataire(id) {
  ...
  if ($('r-locataire-search')) $('r-locataire-search').value = l.prenom + ' ' + l.nom + (l.ville ? ' — ' + l.ville : '');
  // Remplir les champs
  if ($('r-locataire-id')) $('r-locataire-id').value = l.id;
  ...
  if ($('r-locataire-details')) $('r-locataire-details').style.display = 'block';
  // Remplir adresse intervention si locataire a une adresse
  if (l.adresse) {
...
function clearLocataireSelection() {
  if ($('r-locataire-search')) $('r-locataire-search').value = '';
...
function populateLocataireSelectRapport(selectedId) {
  // Compatibilité — avec autocomplétion on n'a plus de select
  // On restaure juste les données si un ID est fourni
  if (!selectedId) return;
  const l = DB.locataires.find(x => x.id === selectedId);
  if (l) {
    if ($('r-locataire-id')) $('r-locataire-id').value = l.id;
    if ($('r-locataire-search')) $('r-locataire-search').value = l.prenom + ' ' + l.nom + (l.ville ? ' — ' + l.ville : '');
  }
...
function updateContraintes() {
  // Récupère les cases cochées et les combine avec le texte libre
  const checked = [];
  document.querySelectorAll('.contrainte-check:checked').forEach(c => checked.push(c.value));
  // Mettre à jour l'aperçu PDF
  updatePDF();
}

function getContraintesText() {
  const checked = [];
  document.querySelectorAll('.contrainte-check:checked').forEach(c => checked.push('• ' + c.value));
  const libre = $('r-contraintes') ? $('r-contraintes').value.trim() : '';
  const parts = [];
...
function setContraintesFromText(text) {
  if (!text) return;
  // Cocher les cases correspondantes
  document.querySelectorAll('.contrainte-check').forEach(cb => {
    cb.checked = text.includes(cb.value);
  });
  // Mettre le reste dans le texte libre
  let libre = text;
  document.querySelectorAll('.contrainte-check').forEach(cb => {
    libre = libre.replace('• ' + cb.value, '').replace(cb.value, '');
  });
  if ($('r-contraintes')) $('r-contraintes').value = libre.replace(/^\n+|\n+$/g, '').trim();
}

function resetContraintes() {
  document.querySelectorAll('.contrainte-check').forEach(cb => cb.checked = false);
  if ($('r-contraintes')) $('r-contraintes').value = '';
}
...
function editRapport(id) {
  ...
  ['r-contact','r-tel','r-email','r-adresse','r-npa','r-ville','r-localisation','r-batiment','r-noint','r-description','r-origine','r-zones','r-precautions','r-duree','r-montant','r-recommandations','r-rdv'].forEach(id => {
    const el = $(id); const key = id.replace('r-','');
    if (el) el.value = r[key] || '';
  });
  setContraintesFromText(r.contraintes || '');
  ...
  if ($('r-show-sig-tech')) $('r-show-sig-tech').checked = r.showSigTech !== false;
  if ($('r-show-sig-client')) $('r-show-sig-client').checked = r.showSigClient !== false;
  if ($('r-rdv-heure')) $('r-rdv-heure').value = r.rdvHeure || '';
  if ($('r-show-duree')) $('r-show-duree').checked = r.showDuree !== false;
  if ($('r-show-rdv')) $('r-show-rdv').checked = r.showRdv !== false;
  if ($('r-show-garantie')) $('r-show-garantie').checked = r.showGarantie !== false;
  if ($('r-show-garantie-note')) $('r-show-garantie-note').checked = r.showGarantieNote !== false;
  if ($('r-show-precautions')) $('r-show-precautions').checked = r.showPrecautions !== false;
...
function saveRapport(statut) {
  ...
  const r = {
    ...
    showDuree: $('r-show-duree') ? $('r-show-duree').checked : true,
    showRdv: $('r-show-rdv') ? $('r-show-rdv').checked : true,
    showGarantie: $('r-show-garantie') ? $('r-show-garantie').checked : true,
    showGarantieNote: $('r-show-garantie-note') ? $('r-show-garantie-note').checked : true,
    showPrecautions: $('r-show-precautions') ? $('r-show-precautions').checked : true,
    ...
    showSigTech: $('r-show-sig-tech') ? $('r-show-sig-tech').checked : true,
    showSigClient: $('r-show-sig-client') ? $('r-show-sig-client').checked : true,
    ...
    origine: $('r-origine').value, contraintes: getContraintesText(),
    ...
    rdv: $('r-rdv').value,
    rdvHeure: $('r-rdv-heure') ? $('r-rdv-heure').value : '', garantie: $('r-garantie').value, statut,
  };
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Fermer autocomplétion locataire si clic ailleurs
  document.addEventListener('click', e => {
    const results = $('r-locataire-results');
    const search = $('r-locataire-search');
