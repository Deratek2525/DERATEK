...
function generatePDF(rapport, statut) {
  try {
    ...
    function row(key, val, shade) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      const valLines = doc.splitTextToSize(String(val), CW - 60);
      const rowH = Math.max(7, valLines.length * 5 + 4);
      checkPage(rowH + 2);
    ...
    function textBox(text, bgColor) {
      ...
      const lineH = 5;
      const padding = 8;
      // Si trop long pour une page, découper en plusieurs blocs
      const maxLinesPerPage = Math.floor((pageH - y - 20) / lineH);
      if (lines.length > maxLinesPerPage && maxLinesPerPage > 3) {
        // Premier bloc sur la page courante
        const firstLines = lines.slice(0, maxLinesPerPage - 2);
        ...
        y += h1 + 3;
        // Suite sur nouvelle page
        const restLines = lines.slice(maxLinesPerPage - 2);
        checkPage(restLines.length * lineH + padding + 10);
